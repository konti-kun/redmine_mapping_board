...
function create_note(d){
  ...
  const BODY_DIV = NOTE_NODE.append("foreignObject")
  .call(set_size, NOTE_WIDTH-10, NOTE_HEIGHT-10)
  .attr("x",5).attr("y",5)
  .append("xhtml:div").attr("style",DIV_STYLE)

  BODY_DIV.append("div").attr("class","note_id").append("div").attr("style","float:left;").text(d["number"]);
  BODY_DIV.select(".note_id").append("div")
  .style("float","right")
  .append("a")
  ...
  const DEL_BTN = NOTE_NODE.append("g")
...
function update_notes(data, svg){
  const NOTE = svg.selectAll("g.note").data(data, function(d){ return d['number']})
